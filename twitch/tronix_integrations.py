...
async def _query_current_stream_window(broadcaster_id:int, seconds:int|float, error:int|float):
    ...
    return script.wrap_python_value(analytics_window(None if executed.result is None else (r0 if isinstance(r0 := executed.result[0], datetime) else datetime.fromtimestamp(r0, timezone.utc))-timedelta(seconds=error), None))
